...
//        return reSizeImage(uploadPath, originalName, fileData);
    ...
    /** 사진 크기 지정해서 crop 하는 api
     * 저장할 위치가 null이라면 uploadPath에 저장 */
   //TODO
    public static Map<String,String> resizeImage(String uploadPath, String originalName,int targetWidth, int targetHeight,String copyPath) throws IOException {

        copyPath = copyPath == null  ? uploadPath :copyPath;

        byte[] bytes = null;
        try(InputStream fs = new FileInputStream(uploadPath + originalName)) {
            //apache commons io
            bytes = IOUtils.toByteArray(fs); //
        }
        reSizeImage(uploadPath,originalName, bytes,targetWidth,targetHeight,copyPath);
    ...
    //TODO 이게진짜 리사이징 이미지인듯
    public static final Map<String,String> reSizeImage(String uploadPath, String originalName, byte[] fileData,int targetWidth, int targetHeight,String copyPath) throws IOException {
        // UUID 발급
...
//		fos.close();
        ...
        BufferedImage destImg = Scalr.resize(cropedImg,targetWidth,targetHeight ,null);
        // 썸네일의 이름을 생성 ( 원본 파일명에 's_'를 붙임)
        String thumbnailName = copyPath + "s_" + fileName;
...
//
    ...
    // 날짜별 디렉토리 추출
    private static final String calcPath(String uploadPath) {
    ...
    // 디렉토리 생성
    private static final void makeDir(String uploadPath, String... paths) {
        // 디렉토리가 존재하면
